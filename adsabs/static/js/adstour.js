...
{	var adsTour = {};
	...
	adsTour.appendToQ = function(string) {
							$("input#q").val(string)
							}

	adsTour.removeFromQ = function() {
							$("input#q").val('')
							}


	adsTour.makeTour = function()
	{
		


		if (currentPage.match(/^http:\/\/(labs.adsabs.harvard.edu\/adsabs|0.0.0.0:5000)\/$/))
			// it's the homepage
			{ 
				...
				// Instance the tour
				// Storage is false to create a new tour on each #tour click event
				adsTour.tour = new Tour({backdrop:false, storage:false});
				...
				adsTour.tour.addSteps([
				  ...
				  {
				    element: "#button_feedback_widget", 
				    title: "Thank You!", 
		...
		else if (currentPage.match(/http:\/\/(labs.adsabs.harvard.edu\/adsabs|0.0.0.0:5000)\/search\/\?/))
			//it's a search results page
			{ 
					adsTour.tour = new Tour({backdrop:true, storage:false});
		...
		else if (currentPage.match(/^http:\/\/(labs.adsabs.harvard.edu\/adsabs|0.0.0.0:5000)\/abs\/.{19}\/$/))
			// abstract page
			{ 
				adsTour.tour = new Tour({backdrop:false, storage:false});
				...
				adsTour.tour.addSteps([
				  ...
				  },
				  {
				    ...
				    title: "Bibcode", 
				    content: "The ADS uses bibliographic codes (bibcodes) to identify literature in our database. <a href=\"http://labs.adsabs.harvard.edu/adsabs/page/help/Filter\">Learn how an ADS bibcode is constructed.</a>",
				    placement:'right'
				  },
				  {
					element: "a.btn.dropdown-toggle:contains(Analyze)", 
	...
	else if (currentPage.match(/^http:\/\/(labs.adsabs.harvard.edu|0.0.0.0:5000)\/search\/classic-search$/))
			// classic search page
			{
				// Instance the tour
				adsTour.tour = new Tour({backdrop:false, storage:false});
	...
	if (adsTour.tour)
		{ 
			// Initialize the tour
			adsTour.tour.init(true);

			// Start the tour		
			adsTour.tour.start(true);
		}
	}

	// disable tour button if it doesn't match one of four tour-having pages
	//	else, add on click listener

	var currentPage = window.location.href;

	if (currentPage.match(/^http:\/\/(labs.adsabs.harvard.edu|0.0.0.0:5000)\/search\/classic-search$/)
		|| currentPage.match(/^http:\/\/(labs.adsabs.harvard.edu\/adsabs|0.0.0.0:5000)\/abs\/.{19}\/$/)
		|| currentPage.match(/^http:\/\/(labs.adsabs.harvard.edu\/adsabs|0.0.0.0:5000)\/search\/\?/)
		|| currentPage.match(/^http:\/\/(labs.adsabs.harvard.edu\/adsabs|0.0.0.0:5000)\/$/))
		{ 
				
				$("#ADS_nav").append('<li><a id="tour" href=""><i class="icon-globe"></i> Tour this Page</a></li>')
				.find("#tour")
				.on("click", function(e)
				{ 
					adsTour.makeTour();	
